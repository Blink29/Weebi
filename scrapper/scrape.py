...
def get_anime_links(page_no):
    while True:
        try:
            request = requests.get(f"https://ww1.gogoanimes.fi/anime-list.html?page={page_no}")
            ...
            for anime_links in anime_list.find_all('a'):
                ...
                link = f"https://ww1.gogoanimes.fi/{anime_link}"
                links.append(link)
...
def get_episode_links(anime_link):
    ...
    episodes = [ "https://ww1.gogoanimes.fi/" + element["href"].strip() for element in soup.find_all('a') ]
    return episodes
